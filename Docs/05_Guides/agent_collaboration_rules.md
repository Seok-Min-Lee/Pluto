...
| **답변 언어** | 모든 아티팩트(구현 계획서, 가이드, 보고서 등)는 한국어로 작성 |
...
- **물리 속성:** 유니티 6의 최신 리니어 물리 API(`linearVelocity`, `linearDamping` 등)를 우선 사용
...
> [!CAUTION]
> **4-6. MCP 전용 조작 원칙 (MCP-Only Operations)**
> 모든 파일 수정, 에셋 조작, 유니티 에디터 제어는 반드시 **공식 Unity MCP 프로토콜**을 통해서만 수행하며, 에디터의 동기화 레이어를 우회하는 로컬 파일 직접 수정(`write_to_file` 등)을 엄격히 금지합니다.
